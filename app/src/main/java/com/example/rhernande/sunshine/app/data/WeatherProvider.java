...
//                    getContext().getContentResolver().notifyChange(uri, null);
    ...
    public int update(
        ...
        switch (match) {
            case WEATHER: {
                normalizeDate(values);
                totalRecords = db.update(WeatherContract.WeatherEntry.TABLE_NAME, values, selection, selectionArgs);
                break;
            }
            case LOCATION: {
                totalRecords = db.update(WeatherContract.LocationEntry.TABLE_NAME, values, selection, selectionArgs);
                break;
            }
            default:
                throw new UnsupportedOperationException("Unknown URI: " + uri);
        ...
        if (totalRecords > 0 || selection != null) {
            getContext().getContentResolver().notifyChange(uri, null);
        }
        db.close();
